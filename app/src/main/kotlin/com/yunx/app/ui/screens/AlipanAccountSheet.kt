...
/**
 * 已登录阿里云盘账号的底部弹窗：展示用户信息、登录时间、refresh_token（可展开/复制），并提供退出登录。
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun AlipanAccountSheet(
    account: AlipanAccountEntity,
    onLogout: () -> Unit,
    onDismiss: () -> Unit
) {
    ...
    val tokenTruncated = account.refreshToken.length > tokenPreviewLimit
    val displayToken = if (showFullToken || !tokenTruncated) {
        account.refreshToken
    } else {
        account.refreshToken.take(tokenPreviewLimit) + "…"
    }
                    ...
                    Row(
                        ...
                        TextButton(
                            onClick = {
                                ...
                                clipboard.setPrimaryClip(ClipData.newPlainText("alipan_refresh_token", account.refreshToken))
                                SnackbarController.show("refresh_token 已复制")
                            }
                        ) {
                            Icon(
    ...
    if (showLogoutConfirm) {
        AlertDialog(
            ...
            title = { Text("退出登录") },
            text = { Text("确定要退出当前阿里云盘账号吗？退出后将清除本地凭证。") },
            confirmButton = {
                TextButton(
                    onClick = {
